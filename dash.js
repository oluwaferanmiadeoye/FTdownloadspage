const downloadsData = [
  ...
  { id: 4, title: 'NEXT SUNDAY AT FAITH TABERNACLE WILL BE OUR COVENANT DAY OF FAVOUR', date: '2025-04-08', type: 'Next', thumbnail: 'Images/events/dayoffavour.png', downloadUrl: 'https://faithtabernacle.org.ng/2025/CovenantDayofFavour.pdf?v=2.4' },
  ...
  { id: 25, title: 'PROVEN STRATEGIES FOR CELL GROWTH AND REPLICATION', date: '2025-01-20', type: 'Inspirational', thumbnail: 'replication.png', downloadUrl: 'https://faithtabernacle.org.ng/2025/ProvenStrategiesforCellGrowthandReplication.pdf?v=2.4' },
...
if (searchInput && clearBtn) {
  ...
  searchInput.addEventListener('input', function () {
    ...
    renderDownloadsTable(this.value.trim());
  });
}
...
function initializeDummyData() {
  ...
  let downloadTimestamps = JSON.parse(localStorage.getItem('downloadTimestamps') || '[]');

  if (downloadedFiles.length === 0) {
    const dummyFiles = [1, 2, 5, 9, 12, 15, 20, 25, 30, 33, 35, 38, 40, 42, 45];
    const dummyTimestamps = dummyFiles.map((id, index) => ({
      id,
      timestamp: new Date(2025, 5, 13 - index, 10, 0, 0).toISOString() // June 13 to June 1, 2025
    }));

    localStorage.setItem('downloadedFiles', JSON.stringify(dummyFiles));
    localStorage.setItem('downloadTimestamps', JSON.stringify(dummyTimestamps));
  }
}
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  if (!downloadsScroller || !downloadsTable) {
    console.error("Required scroller or table elements not found.");
    return;
  }
  initFilters();
  renderFilteredCards();
  ...
  const downloadedTable = document.getElementById('downloadedTable');
  ...
  try {
    ...
    populateTable(downloadedTable, downloadedFiles);
...
function renderDownloadsTable(searchTerm = '') {
  const downloadsTable = document.getElementById('downloadsTable');
  const paginationControls = document.getElementById('pagination-controls');
  if (!downloadsTable || !paginationControls) return;

  let downloadedFiles = JSON.parse(localStorage.getItem('downloadedFiles') || '[]');
  let filteredData = downloadsData.filter(item => downloadedFiles.includes(item.id));
  ...
  populateTable(downloadsTable, paginatedData.map(item => item.id), true);
...
function renderPagination(totalPages) {
  ...
  prevBtn.addEventListener('click', () => {
    if (currentPage > 1) {
      currentPage--;
      renderDownloadsTable(searchInput?.value?.trim() || '');
    }
  });
...
function openDownloadModal(itemId, source) {
  const dataSource = downloadsData;
  ...
  if (!modal || !modalThumbnail || !modalSubtitle || !downloadBtn || !addToLibraryBtn) return;
  ...
  downloadBtn.onclick = () => {
    downloadFile(item.downloadUrl, item.title);
    let downloadedFiles = JSON.parse(localStorage.getItem('downloadedFiles') || '[]');
    if (!downloadedFiles.includes(item.id)) {
      downloadedFiles.push(item.id);
      localStorage.setItem('downloadedFiles', JSON.stringify(downloadedFiles));
      let timestamps = JSON.parse(localStorage.getItem('downloadTimestamps') || '[]');
      timestamps.push({ id: item.id, timestamp: new Date().toISOString() });
      localStorage.setItem('downloadTimestamps', JSON.stringify(timestamps));
      renderDownloadsTable();
    }
  ...
  addToLibraryBtn.onclick = () => {
    ...
    if (!libraryFiles.includes(item.id)) {
      libraryFiles.push(item.id);
...
const tabs = {
  ...
  downloads: { tab: downloadsTab, table: downloadedTable },
